...
namespace ImperaPlus.GeneratedClient
{
    ...
    {
        public System.Threading.Tasks.Task<LoginResponseModel> LoginAsync(string grant_type, string username,
            string password, string scope, string refresh_token)
        {
            return LoginAsync(grant_type, username, password, scope, refresh_token,
        ...
        public async System.Threading.Tasks.Task<LoginResponseModel> LoginAsync(string grant_type, string username,
            string password, string scope, string refresh_token, System.Threading.CancellationToken cancellationToken)
        {
                ...
                using (var request_ = new System.Net.Http.HttpRequestMessage())
                {
                    ...
                    try
                    {
                        ...
                        if (status_ == "400")
                        {
                            ...
                            {
                                throw new ImperaPlusException("Could not deserialize the response body.",
                                    (int)response_.StatusCode, responseData_, headers_, exception);
                            }

                            throw new ImperaPlusException<ErrorResponse>("Client Error", (int)response_.StatusCode,
                                responseData_, headers_, result_, null);
                        }
                        else if (status_ == "200")
                        {
                            var responseData_ = await response_.Content.ReadAsStringAsync().ConfigureAwait(false);
                            ...
                            {
                                result_ =
                                    Newtonsoft.Json.JsonConvert.DeserializeObject<LoginResponseModel>(responseData_);
                                return result_;
                            }
                            catch (System.Exception exception)
                            {
