...
namespace ImperaPlus.Application.Notifications
{
        ...
        : ICompletedEventHandler<TurnEndedEvent>, ICompletedEventHandler<PlayerSurrenderedEvent>
    {
        ...
        public void Handle(TurnEndedEvent evt)
        {
            ...
            this.backgroundJobClient.Enqueue<NotificationJob>(p => p.SendToUser(userId, notification));

            // Notify all active users in game
            var gameId = evt.Game.Id;
            var turnEndedNotification = new DTO.Notifications.TurnEndedNotification { GameId = evt.Game.Id, NewPlayerId = evt.Game.CurrentPlayerId.GetValueOrDefault() };

            // Send in current request
            this.gameNotificationService.SendNotification(gameId, turnEndedNotification);
        }
        ...
        public void Handle(PlayerSurrenderedEvent evt)
        {
            // Notify all active users in game
            var gameId = evt.Game.Id;
            DTO.Notifications.Notification notification = new DTO.Notifications.PlayerSurrenderedNotification { GameId = evt.Game.Id };

            // Send in current request
            this.gameNotificationService.SendNotification(gameId, notification);
        }
    }    
}
